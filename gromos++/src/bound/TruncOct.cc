    ...
    if(refsol.numPos()==sol.numPos())
         for(int i=0;i<sol.numPos();i+= sol.topology().numAtoms()){
            ...
            std::cout << "# solv num " << sol.numPos()
                    ...
                    << " are not equal. solv gathering based on cog of solute : " << std::endl;
...
void TruncOct::gathergr(){
    ...
    if (sys().box().K().abs() == 0 || sys().box().L().abs() == 0 || sys().box().M().abs() == 0) throw gromos::Exception("Gather problem",
                              "Box block contains element(s) of value 0.0! Abort!");
