...
void SolvSolv(const Arguments &args, double time, double dt, double maxdist, double minangle){
//  read topology
   args.check("topo",1);
...
Boundary *pbc = BoundaryParser::boundary(sys, args);
  ...
  vector<int> Hm1, Hm2, Am1, Am2, Htom1, Htom2;  
   for (int j=0;j<sys.sol(0).topology().numAtoms();++j){  
	 ...
	 Hm1.push_back(j); Neighbours neigh(sys,0,j,0); Htom1.push_back(neigh[0]); 
       }
  ...
  //build donor, acceptor list, spit it out
      cout << "Doing a SOLVENT <-> SOLVENT H-bond analysis" << endl;
   ...
   // loop over single trajectory
    ...
    for (int a=0,na=sys.sol(0).topology().numAtoms(), tna=sys.sol(0).numCoords();a<tna;a+=na){
     for (int b=0,nb=sys.sol(0).topology().numAtoms(), tnb=sys.sol(0).numCoords();b<tnb;b+=nb){
       if (a != b){
      num=0;
     for(int i=0;i< int (Hm1.size());++i){
      for(int j=0;j<int (Am2.size());++j){
       num +=1; 
       Vec tmp = (sys.sol(0).pos(a+Hm1[i])-sys.sol(0).pos(b+Am2[j])); 
       dist = sqrt(tmp.dot(tmp));
       if (dist <= maxdist){  
        Vec tmpA = (sys.sol(0).pos(b+Am2[j])-sys.sol(0).pos(a+Hm1[i])); 
        Vec tmpB = (sys.sol(0).pos(a+Htom1[i])-sys.sol(0).pos(a+Hm1[i]));
     ...
     for(int i=0;i<int (Hm2.size());++i){
      for(int j=0;j<int (Am1.size());++j){
       numm +=1;nu+=1;  
       Vec tmp = (sys.sol(0).pos(a+Hm2[i])-sys.sol(0).pos(b+Am1[j])); 
       dist = sqrt(tmp.dot(tmp));
       if (dist <= maxdist){  
        Vec tmpA = (sys.sol(0).pos(b+Am1[j])-sys.sol(0).pos(a+Hm2[i])); 
	...
	cout.precision(3);
        cout.setf(ios::right, ios::adjustfield); 
          cout << setw(5) << num  
            //   << setw(3) << m1+1 
	    //   << setw(4) << sys.sol(0).topology().resNum(Hm1[i])+1
            //   << setw(4) << sys.sol(0).topology().resName(sys.sol(0).topology().resNum(Hm1[i])) 
               << "-"
	    //       << setw(3) << m2+1
            //   << setw(4) << sys.sol(0).topology().resNum(Am2[j])+1
            //   << setw(4) << sys.sol(0).topology().resName(sys.sol(0).topology().resNum(Am2[j])) 
               << setw(4) << Htom1[i]+1 
               << setw(4) << sys.sol(0).topology().atom(Htom1[i]).name() 
               << " - "
               << setw(4) << Hm1[i]+1 
               << setw(4) << sys.sol(0).topology().atom(Hm1[i]).name() 
               << " ... "
               << setw(4) << Am2[j]+1   
	...
	cout.precision(3);
        cout.setf(ios::right, ios::adjustfield); 
          cout << setw(5) << num  
            //   << setw(3) << m2+1 
	    //   << setw(4) << sys.sol(0).topology().resNum(Hm2[i])+1
            //   << setw(4) << sys.sol(0).topology().resName(sys.sol(0).topology().resNum(Hm2[i])) 
               << "-"
            //   << setw(3) << m1+1
            //   << setw(4) << sys.sol(0).topology().resNum(Am1[j])+1
            //   << setw(4) << sys.sol(0).topology().resName(sys.sol(0).topology().resNum(Am1[j])) 
               << setw(4) << Htom2[i]+1 
               ...
               << " ... "
               << setw(4) << Am1[j]+1   
               << setw(4) << sys.sol(0).topology().atom(Am1[j]).name() 
