 ...
 */
std::set<utils::Vertex> utils::FfExpertGraph::get_neighbors(const utils::Vertex &v) const {
 ...
 */
utils::FfExpertGraph utils::FfExpertGraph::cut_subgraph(
        const utils::Vertex &centre, size_t radius) const {
  ...
  for(size_t i = 0; i < radius; ++i) {
    for(std::set<utils::Vertex>::const_iterator it = level[i].begin(),
            to = level[i].end(); it != to; ++it) {
      
      // only insert if it is not there yet!
      if (std::find(g.vertices().begin(), g.vertices().end(), *it) == g.vertices().end())
        g.vertices().push_back(*it);
      
      std::set<utils::Vertex> neighbors = get_neighbors(*it);
      for(std::set<utils::Vertex>::const_iterator neighbor_it = neighbors.begin(),
            neighbor_to = neighbors.end(); neighbor_it != neighbor_to; ++neighbor_it) {
        
        // add it to the next level if it was not considered yet.
        if (std::find(g.vertices().begin(), g.vertices().end(), *neighbor_it) == g.vertices().end())
...
bool utils::FfExpertGraph::match_neighbors(
  ...
  std::copy(neighbors_v1.begin(), neighbors_v1.end(), std::ostream_iterator<utils::Vertex>(std::cout));
  std::cout << std::endl;
  std::cout << "Neighbors 2: ";
  std::copy(neighbors_v2.begin(), neighbors_v2.end(), std::ostream_iterator<utils::Vertex>(std::cout));
  ...
  for (v_set::const_iterator it_v1 = neighbors_v1.begin(), to_v1 = neighbors_v1.end();
          it_v1 != to_v1; ++it_v1) {

    for (v_set::const_iterator it_v2 = neighbors_v2.begin(), to_v2 = neighbors_v2.end();
            it_v2 != to_v2; ++it_v2) {
      ...
      if (utils::FfExpertGraph::match_neighbors(*it_v1, g1, *it_v2, g2, criterion, radius, v1, v2, my_radius + 1)) {
 ...
 */
utils::FfExpertGraph::FfExpertGraph(const FfExpertGraphMapper & mapper, gcore::BbSolute const & bb) {
  for(int i = 0; i < bb.numAtoms(); ++i) {
    const gcore::AtomTopology & atom = bb.atom(i);
    vertices().push_back(utils::Vertex(i, mapper.map_mass(int(atom.mass())), atom.iac(), 
            atom.charge(), atom.name(), bb.resName()));
  }
...
utils::FfExpertGraph::FfExpertGraph(const std::vector<std::string> & atom_name,
  ...
  assert(atom_name.size() == elements.size() && atom_name.size() == order.size());
  // add vertices
  for(size_t i = 0; i < atom_name.size(); ++i) {
    vertices().push_back(utils::Vertex(i,
            utils::FfExpertGraphMapper::str2at(elements[order[i]]), atom_name[order[i]]));
  }
  // add bonds
  ...
  it != to; ++it) {
    ...
    double bond_type;
    switch(it->type()) {
      case 1 : bond_type = 1.0; break;
      case 2 : bond_type = 2.0; break;
      case 4 : bond_type = 1.5; break;
      case 3 : bond_type = 3.0; break;
      default:
        throw gromos::Exception("FfExpertGraph", "unkown bond type (1..4, 4 aromatic)");
    }
...
utils::FfExpertGraphMapper::FfExpertGraphMapper(const std::string & filename) {
  ...
  if (buffer[0] != "FORCEFIELD")
    throw gromos::Exception("MassElementMapper",
  ...
  for (size_t i = 1; i < buffer.size() - 1; i++) {
    int mass; std::string element;
    std::istringstream ss(buffer[i]);
    ss >> mass >> element;
    if (ss.fail())
      throw gromos::Exception("FfExpertGraphMapper", "bad line in ELEMENTMAPPING"
          " block.");
...
utils::Vertex::atom_type_t utils::FfExpertGraphMapper::map_mass(const int & mass) const {
  std::map<int, utils::Vertex::atom_type_t>::const_iterator it = m_mass_map.find(mass);
  if (it == m_mass_map.end()) {
    std::ostringstream msg; msg.precision(8);
...
utils::Vertex::atom_type_t utils::FfExpertGraphMapper::str2at(const std::string & str) {
  if (str == "H") return utils::Vertex::at_H;
  if (str == "C") return utils::Vertex::at_C;
  if (str == "N") return utils::Vertex::at_N;
  if (str == "O") return utils::Vertex::at_O;
  if (str == "F") return utils::Vertex::at_F;
  if (str == "NA") return utils::Vertex::at_Na;
  if (str == "MG") return utils::Vertex::at_Mg;
  if (str == "SI") return utils::Vertex::at_Si;
  if (str == "P") return utils::Vertex::at_P;
  if (str == "S") return utils::Vertex::at_S;
  if (str == "CL") return utils::Vertex::at_Cl;
  if (str == "AR") return utils::Vertex::at_Ar;
  if (str == "CA") return utils::Vertex::at_Ca;
  if (str == "FE") return utils::Vertex::at_Fe;
  if (str == "CU") return utils::Vertex::at_Cu;
  if (str == "ZN") return utils::Vertex::at_Zn;
  if (str == "BR") return utils::Vertex::at_Br;
