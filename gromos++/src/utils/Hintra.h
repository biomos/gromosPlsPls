...
void Intramolecular(const Arguments &args, int molecule, double time, double dt, double maxdist, double minangle){
//  read topology
   args.check("topo",1);
  ...
  //build donor, acceptor list, spit it out
   ...
   if (Acceptors.size() == 0) {throw gromos::Exception("Hbond", "No Acceptors found!\n");}
	...
	if (hto[i] != Acceptors[j]){
      ...
      cout << "Doing an INTRAmolecular H-bond analysis using molecule" << molecule+1 
...
void natIntramolecular(){}

void natIntermolecular(){}
