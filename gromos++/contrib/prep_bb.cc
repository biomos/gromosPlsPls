...
std::string read_pdb(std::string file, std::vector<std::string> &atom,
                     std::vector<std::string> &element, std::vector<Bond> &bond,
                     double bondbound);
/*
string read_mol(string file, vector<string> &atom, vector<string> & element,
vector<Bond> & bond);
*/
std::set<int> neighbours(int a, std::vector<Bond> &bond);
void forward_neighbours(int a, std::vector<Bond> &bond, std::set<int> &cum,
                        int prev);
void add_neighbours(int i, std::vector<Bond> &bond, std::set<int> &at,
                    std::vector<int> &order);
int count_bound(int i, std::vector<Bond> &bond, std::set<int> &at,
                std::set<int> &j);
std::set<int> ring_atoms(std::vector<Bond> &bond, int numatoms);

std::string &operator++(string &s) { return s = s + "  "; }

std::string &operator--(string &s) { return s = s.substr(0, s.length() - 2); }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::vector<Bond> bondnew;
    for (const auto &bd : bond) {
      Bond b(newnum[bd[0]], newnum[bd[1]]);
      bondnew.push_back(b);
    }
    ...
    for (size_t i = 0; i < order.size(); i++) {
      ...
      if (interact) {
          ...
          if (found) {
            ...
            for (size_t radius = hits.size(); radius-- > 0;) {
              ...
              std::map<double, int>::reverse_iterator it = sorted_stat.rbegin(),
                                                      to = sorted_stat.rend();
              for (; it != to; ++it) {
                std::cerr << (it->second + 1) << " : " << setw(5) << it->first
                          << " %  ";
              }
        ...
        if (iac < 0 || iac >= gff->numAtomTypeNames()) {
          std::cerr << "\033[1;31mThis IAC (" << iac + 1 << ") is not "
        ...
        if (gff->findMass(mass) == 0.0) {
          std::cerr << "\033[1;31mThis Masstype (" << mass + 1 << ") is "
        ...
        if (has_graph) {
          ...
          if (found) {
            ...
            for (size_t radius = hits.size(); radius-- > 0;) {
              ...
              std::map<double, double> sorted_stat =
                  swap_key_value(stat[radius]);
              map<double, double>::reverse_iterator it = sorted_stat.rbegin(),
                                                    to = sorted_stat.rend();
              for (; it != to; ++it) {
                std::cerr.precision(4);
                std::cerr << it->second;
        ...
        if (!ocList.empty()) {
          ...
          for (size_t i = 0; i < ocList.size(); ++i) {
            ...
            std::cerr << std::setw(10) << exp.charge(ocList[i].type)
                      << " occurs " << std::setw(3) << ocList[i].occurence
                      << " times\n";
      ...
      for (int inb : nb1) {
        exclusions.insert(newnum[inb]);
        nb2 = neighbours(inb, bond);
        for (int jnb : nb2) {
          exclusions.insert(newnum[jnb]);
      ...
      for (int inb : nb3) {
        if (!exclusions.count(newnum[inb])) {
          exclusions14.insert(newnum[inb]);
        }
      }
    ...
    if (interact) {
      ...
      std::cerr << "\033[1;34m======= IS WRITTEN TO FILE "
                   "============================="
                << "======================\n\n\n";
    ...
    for (size_t i = 0; i < bondnew.size(); i++) {
      if (interact) {
        ...
        if (bt < 1 || bt > gff->numBondTypes()) {
          std::cerr << "\033[1;31mThis Bondtype (" << bt << ") is not "
                    << "defined in the given parameter file\033[22;0m\n";
        }
    ...
    std::vector<Improper> newimpropers;
    std::vector<Dihedral> newdihedrals;
    ...
    for (const auto &bond : bondnew) {
      ...
      set<int> nb1 = neighbours(i0, bondnew);
      nb1.erase(i1);
      set<int> nb2 = neighbours(i1, bondnew);
      nb2.erase(i0);
      ...
      if (!nb1.empty() && !nb2.empty()) {
        if (aro.count(i0) && aro.count(i1)) {
          ...
          newimpropers.push_back(Improper(a, i0, i1, b));
        } else {
          newdihedrals.push_back(Dihedral(*nb1.begin(), i0, i1, *nb2.begin()));
        }
      }
    }
    ...
    for (auto &angle : newangles) {
      if (interact) {
        ...
        Angle iacangle(bb.atom(i0).iac(), bb.atom(i1).iac(), bb.atom(i2).iac());
        ...
        if (bt < 1 || bt > gff->numAngleTypes()) {
          std::cerr << "\033[1;31mThis Angletype (" << bt << ") is not "
                    << "defined in the given parameter file\033[22;0m\n";
        }
    ...
    for (auto &improper : newimpropers) {
      if (interact) {
        ...
        Improper iacimproper(bb.atom(i0).iac(), bb.atom(i1).iac(),
        ...
        if (bt < 1 || bt > gff->numImproperTypes()) {
          std::cerr << "\033[1;31mThis Impropertype (" << bt
                    << ") is not defined in the given "
                    << "parameter file\033[22;0m\n";
        }

        if (gff->improperType(bt - 1).q0() != 0) {
          std::cerr << "\033[1;31mYou might want to modify the order of "
    ...
    for (auto &dihedral : newdihedrals) {
      if (interact) {
        ...
        int i2 = dihedral[2];
        int i3 = dihedral[3];
        Dihedral iacdihedral(bb.atom(i0).iac(), bb.atom(i1).iac(),
                             bb.atom(i2).iac(), bb.atom(i3).iac());
        std::cerr << "\n\033[1;34m"
                  << "--------------------------------------------------------"
                  << "----------------------\n";
        ...
        if (bt < 1 || bt > gff->numDihedralTypes()) {
          std::cerr << "\033[1;31mThis Dihedraltype (" << bt
                    << ") is not defined "
...
string read_pdb(string file, vector<string> &atom, vector<string> &element,
                vector<Bond> &bond, double bondbound) {
...
void add_neighbours(int i, vector<Bond> &bond, set<int> &at,
                    vector<int> &order) {
  ...
  for (map<int, int>::iterator mit = cb.begin(), mto = cb.end(); mit != mto;
       ++mit) {
