...
int main(int argc, char **argv) {
  ...
  try {
    ...
    string s_disres, s_dihres, s_angres, s_jvalue, s_order, s_sym, s_ledih, s_leumb, s_bsleus;
    string s_colvarres;
    ...
    for (; iter != to; ++iter) {
      ...
      if (gin.angleres.found) {
        ...
        if (gin.angleres.thelin < 0.0 || gin.angleres.thelin > 180.0) {
          stringstream read;
          read << gin.angleres.thelin;
          printIO("ANGLERES", "THELIN", read.str(), "0..180");
        }
...
void setParam(input &gin, jobinfo const &job) {
  ...
  for (; iter != to; ++iter) {
    ...
    else if (iter->first == "THELIN")
      gin.angleres.thelin = atof(iter->second.c_str());
