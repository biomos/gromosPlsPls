...
int main(int argc, char **argv) {
  ...
  try {
    ...
    int l_disres = 0, l_dihres = 0, l_angres = 0, l_jvalue = 0, l_order = 0, l_sym = 0, l_ledih = 0, 
        l_zaxisoribias = 0;
    int l_friction=0, l_leumb = 0, l_bsleus = 0, l_qmmm = 0, l_pttopo = 0, l_gamd = 0, l_tfrdc = 0, l_rdc=0;
	...
	if(gin.distanceres.forcescale < 0 || gin.distanceres.forcescale > 2){
      ...
      if (gin.tfrdcres.found) {
        ...
        if (gin.tfrdcres.nttfrdca < 0 || gin.tfrdcres.nttfrdca > 1) {
          stringstream read;
          read << gin.tfrdcres.nttfrdca;
          printIO("TFRDCRES", "NTTFRDCA", read.str(), "0..1");
        }
        if (gin.tfrdcres.ctfrdc < 0) {
          stringstream read;
          read << gin.tfrdcres.ctfrdc;
          printIO("TFRDCRES", "CTFRDC", read.str(), ">=0.0");
        }
        if (gin.tfrdcres.tfrdclin < 0) {
          stringstream read;
          read << gin.tfrdcres.tfrdclin;
          printIO("TFRDCRES", "TFRDCLIN", read.str(), ">=0.0");
        }
        if (gin.tfrdcres.taur < 0) {
          stringstream read;
          read << gin.tfrdcres.taur;
          printIO("TFRDCRES", "TAUR", read.str(), ">=0.0");
        }
        if (gin.tfrdcres.taut < 0) {
          stringstream read;
        ...
        if (gin.tfrdcres.mfvlin < 0) {
          stringstream read;
          read << gin.tfrdcres.mfvlin;
          printIO("TFRDCRES", "MFVLIN", read.str(), ">=0.0");
        }
...
void setParam(input &gin, jobinfo const &job) {
  ...
  for (; iter != to; ++iter) {
    ...
    else if (iter->first == "TFRDCLIN")
      gin.tfrdcres.tfrdclin = atof(iter->second.c_str());
    ...
    else if (iter->first == "MFVLIN")
      gin.tfrdcres.mfvlin = atof(iter->second.c_str());
