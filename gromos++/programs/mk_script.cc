  ...
  try {
	...
	if (!gromosXX) { // Ignore md++ specific blocks
	  ...
	  if (gin.gromos96compat.found) {
	    printWarning("Ignored promd specific block GROMOS96COMPAT\n");
	...
	if(gin.boundcond.found){
	  ...
	  if(l_coord && gin.boundcond.ntb!=0){
	    ...
	    double minbox=1e6;
	    double maxcutoff=0;
	    for(int i=0; i< 3; i++) if(crd.box[i]<minbox) minbox=crd.box[i];
	    if(crd.box.boxformat() == gcore::Box::genbox){
	      minbox=1e6;
	      if(crd.box.K()[0] < minbox) minbox=crd.box.K()[0];
	      if(crd.box.L()[0] < minbox) minbox=crd.box.L()[0];
	      if(crd.box.M()[0] < minbox) minbox=crd.box.M()[0];
	    }
	    
	    if(crd.box.boxformat() == gcore::Box::triclinicbox){
	...
	if(gin.initialise.found){
	  ...
	  if(!gin.stochdyn.found ||
	    ...
	    printError("You are not doing SD, but want to read Stochastic integrals from file (NTISTI in INITIALISE block)");
	  ...
	  for(unsigned int i=0; i< crd.blocks.size(); i++){
	    ...
	    if(crd.blocks[i]=="PERTDATA" && crd.blockslength[i]!=0)
	      pertdata=true;
	    if(crd.blocks[i]=="BOX" && crd.blockslength[i]!=0)
	      box=true;
	    if(crd.blocks[i]=="TRICLINICBOX" && crd.blockslength[i]!=0)
	      box=true;
	  ...
	  if(gin.boundcond.found && gin.boundcond.ntb!=0 && !box)
	    printError("Non-vacuum boundary conditions, but no GENBOX, TRICLINICBOX or BOX block in coordinate file");
	}
	...
	if(gin.positionres.found && gin.positionres.ntpor!=0){
	  ...
	  if(!l_posresspec)
	    ...
	    for(unsigned int i=0; i< posres.blocks.size(); i++){
	      if(!gromosXX && posres.blocks[i]=="POSRESSPEC" && posres.blockslength[i]!=0)
		posresspec=true;
	      if(gromosXX && posres.blocks[i]=="POSRES" && posres.blockslength[i]!=0)
		posresspec=true;
	    }
	    if(!posresspec){
	      std::stringstream ss;
		...
		ss << "No POSRES block in file " << s_posresspec;
		 ...
		 double dt, int ns)
{
  ...
  while(!templates.stream().eof()){
	...
	switch(FILETYPE[sdum]){
	  ...
	  case outtrffile:     names[outtrffile].setTemplate(temp);     break;
