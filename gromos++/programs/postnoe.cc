 ...
 * @date 16.8.2006
 *
 * Program postnoe allows the user to re-analyse the data that was generated by
 * program noe. It reads in the NOE specification file, and the filter-file,
 * which were generated by program @ref prepnoe "prepnoe", as well as the output
 * of program @ref noe "noe". 
 *
 * In cases where a stereospecific hydrogen from a CH2-group, without an
 * explicit assignment was given in the library file of program prepnoe (type 4
 * without subtype), the NOE-distance according to both virtual atoms will have
 * been calculated. Program postnoe can be used to select from these distance
 * the protons that shows either the largest or the smallest violation with the
 * experimental data. Additionally, the user can choose to disregards specific
 * NOE's either by specifying a 0 in as the filter-field in the filter-file that
 * was generated by prepnoe, or by giving a cutoff distance.
 *
 * The user may want to regenerate the filter file using a different kind of
 * pseudo-atom and multiplicity correction by running the prepnoe program a
 * second time. When using postnoe, the reference distances can then be read
 * from this filter-file allowing for a quick assessment of the effect of the
 * corrections. Furthermore, the user can tell postnoe to change the exponential
