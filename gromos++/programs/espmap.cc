  ...
  for (int i=0;i < int (espgrid.size()); ++i){
    ...
    esptmp=0;
               ...
               if (numFrames < 10){
                 ...
                 outpl << outFile <<"_"<<"0000"<< numFrames<<extpl<<ends;
               }
               else if (numFrames < 100){
                 out << outFile<<"_"<<"000"<< numFrames<<ext<<ends;
                 outpl << outFile<<"_"<<"000"<< numFrames<<extpl<<ends;
               }
               else if (numFrames < 1000){
                 out << outFile <<"_"<<"00"<< numFrames<<ext<<ends;
                 outpl << outFile <<"_"<<"00"<< numFrames<<extpl<<ends;
               }
               else {
                out << outFile <<"_"<<"0"<< numFrames<<ext<<ends;
                outpl << outFile <<"_"<<"0"<< numFrames<<extpl<<ends;
	       }
	       //write pdb
               ofstream os(out.str()); 
               oc->open(os);       
