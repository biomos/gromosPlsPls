...
void printInput(string ifile, string ofile, istep step, iperturb perturb, int i, double lam);
...
int main(int argc, char **argv){
  ...
  char *knowns[] = {"sys", "script", "bin", "dir", "queue", 
		    "files", "template", "lambda", "XX"} ;
  int nknowns = 9;

  string usage = argv[0];
  usage += "\n\t@sys  <system name>\n";
  usage += "\t@script <number of equi> <number of production scripts>\n";
  ...
  try{
    ...
    int numEquiScripts=1, numProdScripts=1;
    {
	...
	submitcommand="ssub -s "+q+" ";
      for(iter=args.lower_bound("lambda");
	  iter!=args.upper_bound("lambda");++iter)
	lambda.push_back(atof(iter->second.c_str()));
      iter=args.lower_bound("script");
      if(iter!=args.upper_bound("script")){
	numEquiScripts=atoi(iter->second.c_str());
	++iter;
      }
      if(iter!=args.upper_bound("script"))
	numProdScripts=atoi(iter->second.c_str());
      numScripts=numEquiScripts+numProdScripts;
    }
	...
	nextlsubdir=bos.str();
      ...
      lsubdir=os.str();
      ldir=simuldir+"/"+lsubdir;

      mkdir(ldir.c_str(), 00755);   // mkdir ldir
      chdir(ldir.c_str());  // cd ldir
	...
	fout << "WORKDIR=" << misc[0].name(i) << "_" << lambda[l] << endl;
	fout << "mkdir -p ${WORKDIR}\n";
	fout << "cd       ${WORKDIR}\n";
	fout << "\n# set the input files\n";
	fout << "TOPO=${SIMULDIR}/" << s_topo << endl;
	fout << "IUNIT=${SIMULDIR}/" << lsubdir << "/";
	fout << filenames[FILETYPE["input"]].name(i) << endl;
        printInput(simuldir+"/"+ s_input, filenames[FILETYPE["input"]].name(i), 
		   gin.step,gin.perturb, i, lambda[l]);
	fout << "INPUTCRD=${SIMULDIR}/";
	if(l==0 && i==0 && l_coord) fout << s_coord << endl;
	else {
	  if(i==0) 
	    fout << prevlsubdir << "/" << filenames[FILETYPE["coord"]].name(numEquiScripts-1) << endl;
	  else     
	    fout << lsubdir << "/" 
		 << filenames[FILETYPE["coord"]].name(i-1) << endl;
	}
	...
	fout << "\n# copy the files back\n";
	fout << "OK=1\n";
	fout << setw(25) << "cp ${OUNIT}" 
	     << " ${SIMULDIR}/" << lsubdir << " || OK=0\n";
	fout << setw(25) << "cp ${OUTPUTCRD}" 
	     << " ${SIMULDIR}/" << lsubdir << " || OK=0\n";
	...
	for(unsigned int k=0; k<linkadditions.size(); k++){
	  if(linkadditions[k]>0){
	    ...
	    fout << setw(25) << s << " ${SIMULDIR}/" << lsubdir << " || OK=0\n";
	  }
	}
	...
	if(i==numEquiScripts-1 && l!=lambda.size()-1){
	  fout << "cd ../" << nextlsubdir << endl;
	  fout << misc[1].name(0) << endl;
	}
	
	fout.close();
	chmod(filenames[FILETYPE["script"]].name(i).c_str(), 00755);
      }
    }
  }
...
void printInput(string ifile, string ofile, istep step, iperturb perturb, int i, double lam){
	...
	fin.getline(s);
        if(s=="STEP"){
	    fout << "STEP\n";
	    fout << step.nstlim << "\t" << step.t+i*step.nstlim*step.dt 
		 << "\t" << step.dt << endl;
            fout << "END\n";
	  ...
	  fout << "PERTURB\n";
	  fout << perturb.ntg << "\t" << perturb.nrdgl << "\t" 
	       << lam << "\t" << perturb.dlamt << "\t" << perturb.rmu 
	       << "\t" << perturb.dmut << endl;
	  fout << perturb.alphlj << "\t" << perturb.alphc << "\t" 
	       << perturb.nlam << "\t" << perturb.mmu << endl;
